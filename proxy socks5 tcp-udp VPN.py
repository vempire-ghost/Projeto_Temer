...
class SocksProxy:
    ...
    def handle_udp_associate(self, client_socket, client_addr, dest_addr, dest_port, addr_type):
        try:
            ...
            # 2. Para enviar para internet (na rede 192.168.101.x)
            ...
            # Socket para enviar para internet - bind na interface específica (192.168.101.2)
            send_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            send_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            send_socket.bind((self.bind_ip, 0))  # self.bind_ip = 192.168.101.2
    ...
    def handle_udp_traffic(self, session):
        ...
        send_socket = session['send_socket']  # Envia/recebe da internet (192.168.101.2)
