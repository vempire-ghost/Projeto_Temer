...
log_file_path = os.path.join(log_dir, 'proxy_tcp_udp.log')
...
class SocksProxy:
    ...
    def start_socks_proxy(self):
        ...
        while self.running:
            try:
                ...
                logger.error(f"Erro ao aceitar conexão TCP em {self.bind_ip}:{self.local_port}: {e}")
    ...
    def handle_socks_connection(self, client_socket, client_addr):
        try:
            ...
            if len(handshake) < 2:
                logger.warning(f"[{self.bind_ip}:{self.local_port}] Handshake incompleto")
                client_socket.close()
                return

            version, n_methods = handshake
            if version != 0x05:
                logger.error(f"[{self.bind_ip}:{self.local_port}] Versão SOCKS não suportada: {version}")
                client_socket.close()
                return
            ...
            if len(request) < 4:
                logger.warning(f"[{self.bind_ip}:{self.local_port}] Pedido SOCKS incompleto")
                client_socket.close()
                return
            ...
            if addr_type == 0x01:  # IPv4
                ...
                logger.warning(f"[{self.bind_ip}:{self.local_port}] Tipo de endereço não suportado")
            ...
            logger.info(f"[{self.bind_ip}:{self.local_port}] Pedido SOCKS5: cmd={cmd}, addr={dest_addr}, port={dest_port}")
            ...
            if cmd == 0x01:  # CONNECT (TCP)
                ...
                logger.error(f"[{self.bind_ip}:{self.local_port}] Comando não suportado: {cmd}")
                client_socket.sendall(b'\x05\x07\x00\x01' + socket.inet_aton('0.0.0.0') + b'\x00\x00')
                client_socket.close()

        except Exception as e:
            logger.error(f"[{self.bind_ip}:{self.local_port}] Erro na conexão SOCKS: {e}")
            client_socket.close()
    ...
    def handle_tcp_connection(self, client_socket, addr, port):
        try:
            ...
            logger.error(f"[{self.bind_ip}:{self.local_port}] Erro na conexão TCP: {e}")
            client_socket.close()
...
def run_proxy_instance(port, bind_ip):
    proxy = SocksProxy(port, bind_ip)
    try:
        proxy.start_socks_proxy()
    except Exception as e:
        logger.error(f"Erro ao iniciar proxy na porta {port}: {e}")

if __name__ == '__main__':
    # Limpar arquivo de log no início
    open('proxy_tcp_udp.log', 'w').close()
    ...
    for config in proxy_configs:
        thread = threading.Thread(
            target=run_proxy_instance,
            args=(config["port"], config["bind_ip"])
        )
        thread.daemon = True
        proxy_threads.append(thread)
        thread.start()
        logger.info(f"Proxy iniciado em {config['bind_ip']}:{config['port']}")
    ...
    try:
        ...
        logger.info("Encerrando proxies...")
